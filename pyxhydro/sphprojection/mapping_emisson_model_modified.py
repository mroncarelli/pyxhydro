...
def make_spectrum_cube(iterator, nx, ny, nz, iter_, x, y, hsml, norm, z_eff, temp_kev, metallicity, energy, method,
    ...
    # Use a list to hold the final cube for mutable access
    final_cube_container = [np.zeros((nx, ny, nz), dtype=np.float64)]
    lock = Lock()
    ...
    with Pool(processes=n_jobs) as pool:
        for idx, chunk in enumerate(chunks):
            args = (
                z_eff[chunk], temp_kev[chunk], metallicity[chunk], 
                norm[chunk], x[chunk], y[chunk], hsml[chunk],
                nx, ny, nz, energy, method, flag_ene, 
                f"Chunk {idx + 1} Progress :"
            )
            pool.apply_async(
    ...
    print(final_cube_container[0].shape)
    return final_cube_container[0]  # Return the numpy array from the container
    ...
    with EmissionModels(method, energy) as em:
        for (redshift, temperature, Z, normalization, xi, yi, hsml_i) in zip(z_eff, temp_kev, metals, norm, x, y, hsml):
            ...
            if not (0 <= i0 < nx and 0 <= j0 < ny):
                continue  # Skip particles out of bounds
            if i0 + len(wx) > nx or j0 + len(wy) > ny:
                continue  # Adjust for particles near edges if necessary
...
def make_simput_emission_model(simfile: str, size: float, emin: float, emax: float, bins: int, method: str, npix=256,
                               redshift=None, center=None, radius_halo=None, proj='z', zrange=None, tcut=0.,
                               flag_ene=False,
                               novel=None,
    ...
    if radius_halo is not None:
        radius_halo = radius_halo / size_gadget * npix
    ...
    write_specmap(result, f'pyxhydro-snap.speccube')
